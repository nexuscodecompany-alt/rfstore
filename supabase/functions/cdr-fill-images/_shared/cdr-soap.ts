...
async function callSoap(url: string, method: string, params: Record<string, string>): Promise<string> {
	...
	if (!match) throw new Error(`No se encontro tag SOAP: ${xml.slice(0, 800)}`);
...
export interface CdrProduct { codigo: string; nombre: string; galeria: CdrImage[]; }
...
export async function fetchProductosConGaleria(email: string, token: string, fechaDesde: string): Promise<CdrProduct[]> {
	...
	if (!Array.isArray(parsed)) throw new Error(`Respuesta WS no es array`);

-- Incluye las ventas manuales (channel='manual') en las métricas de costo y
-- ganancia del dashboard. Los ingresos ya entran solos vía total_amount /
-- total_original; acá sumamos el costo + costos extra y la ganancia manual.
...
begin
  select jsonb_build_object(
    ...
    'prev_paid_margin_period', (
      ...
      from orders o join order_items oi on oi.order_id = o.id
      left join variants v on v.id = oi.variant_id left join products pr on pr.id = v.product_id
      where o.payment_status = 'paid' and o.created_at >= v_prev_from and o.created_at < p_from
    ) + (
      select coalesce(sum(coalesce(total_amount,0) - coalesce(manual_cost_usd,0) - coalesce(manual_extra_costs_usd,0)),0)
    ...
    'uyu_cost', (
      ...
      from orders o join order_items oi on oi.order_id = o.id
      left join variants v on v.id = oi.variant_id left join products pr on pr.id = v.product_id
      where o.payment_status='paid' and o.ml_currency='UYU' and o.created_at>=p_from and o.created_at<=p_to
    ) + (
    ...
    'usd_cost', (
      ...
      from orders o join order_items oi on oi.order_id = o.id
      left join variants v on v.id = oi.variant_id left join products pr on pr.id = v.product_id
      where o.payment_status='paid' and coalesce(o.ml_currency,'USD')='USD' and o.created_at>=p_from and o.created_at<=p_to
    ) + (
      select coalesce(sum(coalesce(manual_cost_usd,0) + coalesce(manual_extra_costs_usd,0)),0)
